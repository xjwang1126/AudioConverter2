...
bool Matrix::equals(const Matrix &src) {
    ...
    for (int i = 0; i < MATRIX_SIZE && equals; i++) {
        if (mData[i] > d[i] + 0.01f || mData[i] < d[i] - 0.01f) {
            equals = false;
        }
    }
    return equals;
...
Matrix *Matrix::newLookAt(float eyeX, float eyeY, float eyeZ, float centerX,
    ...
    if (m) {
        ...
        // Normalize f
        float rlf = 1.0f / static_cast<float>(sqrt(fx * fx + fy * fy + fz * fz));
        fx *= rlf;
        fy *= rlf;
        fz *= rlf;
        ...
        // and normalize s
        float rls = 1.0f / static_cast<float>(sqrt(sx * sx + sy * sy + sz * sz));
        sx *= rls;
        sy *= rls;
        sz *= rls;
...
Matrix *Matrix::newRotate(float radians, float x, float y, float z) {
    radians *= static_cast<float>(3.14159265358979323846f / 180.0f);
    Matrix *m = new Matrix();
    if (m) {
        float *d = m->mData;
        d[3] = 0;
        ...
        d[14] = 0;
        d[15] = 1;
        float s = static_cast<float>(sinf(radians));
        float c = static_cast<float>(cosf(radians));
